...
export default function Nutrition() {
  ...
  const downloadPDF = () => {
    ...
    try {
      ...
      doc.text("Generated by Sai Aerobics", pageWidth / 2, finalY + 6, { align: "center" });
      ...
      days.forEach((day, idx) => {
        ...
        html += `
          <div style="background: white; border-radius: 10px; padding: 15px; margin-bottom: 12px; border-left: 4px solid #E85D75;">
            <h3 style="margin: 0 0 10px 0; color: #E85D75;">${dayNames[idx]}</h3>
            <p style="margin: 4px 0;"><strong>Breakfast:</strong> ${d.breakfast || "-"}</p>
            <p style="margin: 4px 0;"><strong>Lunch:</strong> ${d.lunch || "-"}</p>
            <p style="margin: 4px 0;"><strong>Dinner:</strong> ${d.dinner || "-"}</p>
            <p style="margin: 4px 0;"><strong>Snacks:</strong> ${d.snacks || "-"}</p>
          </div>
        `;
      });
        ...
        <div style={{ background: "var(--card)", padding: "24px", borderRadius: "24px", boxShadow: "0 8px 30px rgba(0,0,0,0.04)", maxWidth: "700px", margin: "0 auto" }}>
          {!generatedPlan ? (
            ...
            <>
              ...
              {/* Action Buttons - Only Download and Email */}
                  ...
                  return (
                    <div key={day} style={{ background: "var(--bg)", padding: "20px", borderRadius: "16px", border: "1px solid var(--border)" }}>
                      <h4 style={{ margin: "0 0 15px 0", color: "var(--primary)" }}>
                        <span style={{ background: "var(--primary)", color: "white", padding: "6px 14px", borderRadius: "20px", fontSize: "0.9rem" }}>
                          {dayNames[index]}
                        </span>
                      </h4>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "12px" }}>
                        <div>
                          <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", fontWeight: "600" }}>Breakfast</p>
                          <p style={{ margin: "4px 0 0 0", fontWeight: "500" }}>{d.breakfast || "-"}</p>
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", fontWeight: "600" }}>Lunch</p>
                          <p style={{ margin: "4px 0 0 0", fontWeight: "500" }}>{d.lunch || "-"}</p>
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", fontWeight: "600" }}>Dinner</p>
                          <p style={{ margin: "4px 0 0 0", fontWeight: "500" }}>{d.dinner || "-"}</p>
                        </div>
                        <div>
                          <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", fontWeight: "600" }}>Snacks</p>
